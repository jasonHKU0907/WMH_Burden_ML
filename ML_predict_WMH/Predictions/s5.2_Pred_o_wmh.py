...
outfile = dpath + 'Results/RM_BL_BD/s5_Pred_x24485_2_0.csv'
...
target_df = pd.read_csv(dpath + 'Data/WMH_target.csv', usecols = ['eid', 'x24485_2_0', 'Region_code'])
target_df['target_y'] = target_df['x24485_2_0'].copy()
target_df['target_y'] = np.log(target_df['target_y'])
target_df['target_y'].replace([np.inf, -np.inf], np.nan, inplace = True)
target_df['target_y'].dropna(inplace = True)
target_df['target_y'].describe()
#ubd = target_df['target_y'].mean() + target_df['target_y'].std()*4
#lbd = target_df['target_y'].mean() - target_df['target_y'].std()*4
#target_df['target_y'].iloc[target_df['target_y']>ubd] = np.nan
#target_df['target_y'].iloc[target_df['target_y']<lbd] = np.nan
#target_df['target_y'].dropna(inplace = True)
#target_df.reset_index(inplace = True, drop = True)
#target_df['target_y'] = np.round((target_df['target_y'] - np.mean(target_df['target_y'])) / target_df['target_y'].std(), 5)
